 ...
 * 
 * POST /api/members/workouts/progress - Save workout progress
 * GET /api/members/workouts/progress - Get workout progress
 */
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      progressDate.setHours(0, 0, 0, 0); // Set to start of day
      
      const nextDay = new Date(progressDate);
      nextDay.setDate(nextDay.getDate() + 1); // Next day at midnight
      
      console.log(`Looking for progress between ${progressDate} and ${nextDay}`);
